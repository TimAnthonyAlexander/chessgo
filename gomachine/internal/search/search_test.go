...
// TestIsRepetitionAsymmetric locks the standard asymmetric repetition rule
// (Chess Programming Wiki, "Repetitions"): a repetition strictly AFTER the root
// (a cycle inside the search tree) is a draw on the first occurrence, but a
// repetition reaching into the pre-root game history is only a draw at a genuine
// threefold. A two-fold against the real game must NOT be scored as a draw — the
// bug that let a mated side "escape" into a phantom draw during full-game
// analysis the instant a position recurred (e.g. a king shuffle).
func TestIsRepetitionAsymmetric(t *testing.T) {
	// Wide halfmove clock so the scan window covers the whole keyStack.
	pos, err := chess.ParseFEN("8/8/8/4k3/8/4K3/8/8 w - - 50 80")
	if err != nil {
		t.Fatal(err)
	}
	K := pos.Key()
	F := K ^ 0xABCDEF12 // any key that does not collide with K

	cases := []struct {
		name    string
		rootIdx int
		stack   []uint64 // top element is the current node (its value is unread)
		want    bool
	}{
		{"two-fold vs game history is not a draw", 1, []uint64{K, F, F}, false},
		{"three-fold vs game history is a draw", 3, []uint64{K, F, K, F, F}, true},
		{"two-fold inside the search tree is a draw", 0, []uint64{F, F, K, F, F}, true},
		{"a single repeat of the root is not a draw", 2, []uint64{F, F, K, F, F}, false},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			s := New(1)
			s.keyStack = append(s.keyStack[:0], c.stack...)
			s.rootIdx = c.rootIdx
			if got := s.isRepetition(pos); got != c.want {
				t.Fatalf("isRepetition = %v, want %v", got, c.want)
			}
		})
	}
}

// TestSearchIgnoresTwofoldGameHistoryRepetition is a regression test for the
// analysis bug: a position that occurred ONCE earlier in the game (two-fold vs
// history) was scored 0.00 instead of the real forced mate. We replay the real
// game up to the recurring "rnb2rk1/.../1PPPKqPP/... w" position and assert the
// search still sees that the side to move is being mated.
func TestSearchIgnoresTwofoldGameHistoryRepetition(t *testing.T) {
	moves := []string{
		"g1f3", "g7g6", "e2e4", "f8g7", "b1c3", "c7c6", "a2a4", "e7e5",
		"f1c4", "g8f6", "a1a3", "e8g8", "e1e2", "d7d5", "e4d5", "e5e4",
		"c4b5", "e4f3", "e2e3", "f6g4", "e3f3", "d8f6", "f3e2", "f6f2",
		"e2d3", "f2d4", "d3e2", "d4f2", // reaches the position that first occurred at ply 24
	}
	pos, err := chess.ParseFEN(chess.StartFEN)
	if err != nil {
		t.Fatal(err)
	...
	for _, uci := range moves {
		...
		if !ok {
			t.Fatalf("illegal move in sequence: %s", uci)
		}
		var u chess.Undo
		pos.DoMove(m, &u)
	}

	r := New(16).Search(pos, Limits{Depth: 8}, history)
	if r.MateIn >= 0 {
		t.Fatalf("two-fold-vs-history position scored as non-loss (MateIn=%d, Score=%d); "+
			"expected the side to move to be getting mated (MateIn<0)", r.MateIn, r.Score)
	}
}

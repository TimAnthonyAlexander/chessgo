...
func ParseParams(base search.Params, spec string) (search.Params, error) {
	...
	for _, tok := range strings.Split(spec, ",") {
		...
		case "pcmbaseoffset", "pcmoffset":
			n, err := strconv.Atoi(val)
			if err != nil {
				return base, fmt.Errorf("pcmbaseoffset: %q is not an int", val)
			}
			base.PCMBaseOffset = n
...
func DiffParams(base, patch search.Params) string {
	...
	if base.PCMBaseOffset != patch.PCMBaseOffset {
		diffs = append(diffs, fmt.Sprintf("pcmbaseoffset: %d→%d", base.PCMBaseOffset, patch.PCMBaseOffset))
	}

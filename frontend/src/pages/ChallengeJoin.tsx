      ...
      {failed ? (
        <>
          ...
          <Typography sx={{ color: 'var(--text-dim)', fontSize: 14, maxWidth: 360 }}>
            {s.error ?? 'This invite is no longer available. It may have expired or already started.'}
          </Typography>
          <Box sx={{ display: 'flex', gap: 1, mt: 1 }}>
            <Button variant="contained" onClick={() => navigate('/')}>Back to lobby</Button>
            <Button color="inherit" onClick={() => navigate('/bot')} sx={{ color: 'var(--text-dim)' }}>
              Play the computer
            </Button>
          </Box>
        </>

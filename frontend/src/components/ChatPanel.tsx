  ...
  return (
    ...
    >
      ...
      >
        ...
        <Typography sx={{ fontFamily: 'var(--font-display)', fontSize: 13, fontWeight: 700, letterSpacing: 0.3 }}>
          Chat
        </Typography>
      </Box>

      <Box sx={{ flex: 1, minHeight: 0, overflowY: 'auto', px: 1.5, py: 1.25, display: 'flex', flexDirection: 'column', gap: 0.75 }}>
        {messages.length === 0 ? (
          <Typography sx={{ fontSize: 12.5, color: 'var(--muted)', textAlign: 'center', mt: 1 }}>
            Say hello — be nice.
          </Typography>
        ) : (
      ...
      <Box
        ...
        sx={{ display: 'flex', alignItems: 'center', gap: 1, p: 1, borderTop: '1px solid var(--line-soft)', bgcolor: 'var(--bg-2)' }}
      >
        <Box
          component="input"
          value={text}
...
function ChatLine({ m }: { m: ChatMessage }) {
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: m.mine ? 'flex-end' : 'flex-start' }}>

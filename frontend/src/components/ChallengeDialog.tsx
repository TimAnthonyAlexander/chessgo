...
export default function ChallengeDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const poolValid = (() => {
    ...
    return Number.isFinite(b) && Number.isFinite(i) && b >= 0 && i >= 0 && b <= 180 && i <= 180 && !(b === 0 && i === 0)
  })()
  ...
  return (
    <Dialog
      open={open}
      onClose={close}
      slotProps={{ paper: { sx: { bgcolor: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 3, minWidth: 380, maxWidth: 420 } } }}
    >
          ...
          <>
            ...
            {!custom ? (
              <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 0.75 }}>
                {PRESETS.map((p) => (
                  <Chip key={p} label={p} active={preset === p} onClick={() => setPreset(p)} />
                ))}
                <Chip label="Custom" active={false} onClick={() => setCustom(true)} dashed />
              </Box>
            ) : (
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                ...
                <TextField
                  ...
                  sx={{ flex: 1 }}
                />
                <Button size="small" color="inherit" onClick={() => setCustom(false)} sx={{ color: 'var(--text-dim)', textTransform: 'none' }}>
                  Presets
                </Button>
              </Box>
            )}
            ...
            <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 0.75 }}>
              <Chip label="White" active={color === 'w'} onClick={() => setColor('w')} />
              <Chip label="Random" active={color === 'random'} onClick={() => setColor('random')} />
              <Chip label="Black" active={color === 'b'} onClick={() => setColor('b')} />
            </Box>

            {/* Rated */}
            <Box
              sx={{ mt: 2, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 1 }}
            >
              <Box>
                <Typography sx={{ fontSize: 14, fontWeight: 600 }}>Rated</Typography>
                <Typography sx={{ fontSize: 12, color: 'var(--text-dim)' }}>
                  {loggedIn ? 'Affects both players’ ratings' : 'Log in to play rated'}
                </Typography>
              </Box>
              <Box sx={{ display: 'flex', gap: 0.75 }}>
                <Chip label="Casual" active={!effectiveRated} onClick={() => setRated(false)} small />
                <Chip
                  label="Rated"
            ...
            <Divider sx={{ my: 2.5, borderColor: 'var(--line)', '&::before, &::after': { borderColor: 'var(--line)' } }}>
              <Typography sx={{ fontSize: 11.5, color: 'var(--muted)', letterSpacing: '0.1em' }}>OR JOIN</Typography>
            </Divider>
            ...
            <Box sx={{ display: 'flex', gap: 1 }}>
              <TextField
                ...
                size="small"
                fullWidth
                slotProps={{ htmlInput: { style: { letterSpacing: '0.15em', fontFamily: 'var(--font-mono)' } } }}
              />
              <Button
                variant="outlined"
                onClick={join}
            ...
            {s.error && (
              <Alert severity="error" variant="outlined" sx={{ mt: 1.75, fontSize: 13 }}>
                {s.error}
              </Alert>
            )}
          </>
...
function InviteView({
  ...
  const colorLabel = color === 'w' ? 'You play White' : color === 'b' ? 'You play Black' : 'Random colors'
  return (
    <Box sx={{ textAlign: 'center' }}>
      <Typography sx={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 20, mb: 0.5 }}>
      ...
      >
        <Typography
          sx={{ fontFamily: 'var(--font-mono)', fontSize: 30, fontWeight: 700, letterSpacing: '0.22em', pl: '0.22em' }}
        >
          {code}
        </Typography>
      ...
      >
        <Typography
          sx={{ flex: 1, minWidth: 0, fontSize: 12.5, color: 'var(--text-dim)', textAlign: 'left', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
        >
          {link}
        </Typography>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 1.5, mb: 2.5, color: 'var(--text-dim)', fontSize: 12.5 }}>
        <Crown size={14} />
        <span>{pool}</span>
        <span>·</span>
      ...
      <Button fullWidth color="inherit" onClick={onCancel} sx={{ color: 'var(--text-dim)', textTransform: 'none' }}>
        Cancel invite
      </Button>
    </Box>
  )
...
function Label({ text, sx }: { text: string; sx?: object }) {
  return (
    <Typography sx={{ fontSize: 12, fontWeight: 600, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.08em', mb: 1, ...sx }}>
      {text}
    </Typography>
  )
}
  ...
  return (
    <Box
      ...
      sx={{
        ...
        border: active ? '1px solid var(--accent)' : dashed ? '1px dashed var(--line)' : '1px solid var(--line)',
        transition: 'border-color .12s, background .12s, color .12s',
        '&:hover': disabled ? {} : { borderColor: 'var(--accent-line)', color: active ? '#15171c' : 'var(--text)' },
      }}
    >
      {label}
    </Box>
...
function CopyButton({ active, onClick, label }: { active: boolean; onClick: () => void; label?: boolean }) {
  return (
    <Button
      onClick={onClick}
      size="small"
      ...
      sx={{ flexShrink: 0, minWidth: label ? undefined : 40, px: label ? 1.25 : 1, textTransform: 'none', borderColor: 'var(--line)', color: active ? 'var(--accent)' : 'var(--text-dim)' }}
    >

...
function SectionHeader({ label, to, onNavigate }: { label: string; to?: string; onNavigate: () => void }) {
  const sx = {
    display: 'block',
    px: 2,
    pt: 2,
...
export default function MobileNavDrawer({ sections, user, onLogin, onLogout }: MobileNavDrawerProps) {
      ...
      >
        <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
          ...
          >
            <Typography
              component="span"
              sx={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 20, color: 'var(--text)', letterSpacing: '-0.01em' }}
            >
              chessgo
            </Typography>
            <IconButton aria-label="Close navigation menu" onClick={close} sx={{ color: 'var(--text-dim)' }}>
              <X size={20} />
            </IconButton>
          </Box>
            ...
            {sections.map((section) =>
              section.items && section.items.length > 0 ? (
                <Box key={section.label} sx={{ mb: 0.5 }}>
                  <SectionHeader label={section.label} to={section.to} onNavigate={close} />
                  {section.items.map((item) => (
                    <NavRow key={item.label} label={item.label} to={item.to} onNavigate={close} />
                  ))}
                </Box>
              ) : section.to ? (
                <NavRow key={section.label} label={section.label} to={section.to} onNavigate={close} />
              ) : null,
            )}
          </Box>

...
export default function BoardEditor({
  ...
  function onPointerDown(e: ReactPointerEvent<HTMLDivElement>) {
    ...
    setDrag({ from: sq, piece, startX: e.clientX, startY: e.clientY, x: e.clientX, y: e.clientY, over: sq, size, moved: false })
  }
      ...
      >
        {ranks.map((rank) =>
          files.map((file) => {
            ...
            const classes = ['sq', light ? 'light' : 'dark', isOver ? 'over' : ''].filter(Boolean).join(' ')
            return (
              <div key={sq} className={classes}>
                {piece && (
                  <span
                    className="piece"
                    style={{ backgroundImage: `url(${pieceImageUrl(piece)})`, ...(isDragOrigin ? { opacity: 0 } : {}) }}
                  />
                )}

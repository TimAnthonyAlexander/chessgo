...
- **Dual small/big net** — Stockfish **still runs it** (SF18/SFNNv10 *and* master/SFNNv13: big `1024x2→31(+1)→32→1` with Full-Threat inputs, small `128x2→15(+1)→32→1`, HalfKAv2_hm only, no threats). The earlier "removed in 2025" note was **WRONG** — it came from a *fork*, not real Stockfish. It's a live speed technique (cheap net for materially-decided positions), just not our near-term lever.

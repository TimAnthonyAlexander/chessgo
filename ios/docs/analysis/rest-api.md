...
## ANALYSIS
- `POST /analyze` — body `{fen(required), movetime?, depth?}` → `{eval:{type:"cp"|"mate",value}|null, bestmove:uci|null, pv:[uci]|null, depth:int|null}`. eval side-to-move-relative. Default movetime 1500ms; depth clamped 1..40.
- `POST /candidates` — body `{fen(required), history?:string[](body only), multipv?(≤12), movetime?(50..2000,def300), depth?(≤30)}` → `{opening:{eco,name}|null, moves:[{uci,san,eval,pv,depth}]}` best-first.
